...
ssh $user@$external_ip "printf \"%s\n\" nutanix/4u nutanix/4u | sudo passwd ; sudo iptables -t nat -A PREROUTING -j DNAT --to-destination $destination_ip ; sudo iptables -t nat -A POSTROUTING -j MASQUERADE ; sudo sysctl net.ipv4.ip_forward=1"
